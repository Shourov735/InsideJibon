...
import { AssignmentSubmissionsTable } from "./assignment-submissions-table";
...
/**
 * R0 §4.3: server-friendly orchestrator for the teacher assignment
 * detail page. Owns lifecycle action state (publish/unpublish/close/
 * reopen/delete) and the publish modal. The submissions table and
 * grading drawer are delegated to <AssignmentSubmissionsTable />.
 */
export function AssignmentDetailView({
  ...
  const formattedPublished = assignment.publishedAt
    ? new Intl.DateTimeFormat(locale === "bn" ? "bn-BD" : "en-US", {
        dateStyle: "medium",
      }).format(new Date(assignment.publishedAt))
    : null;
  ...
  const handleDelete = async () => {
    ...
    setActionError(null);
    try {
      const res = await deleteAssignmentAction(
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2.5 shrink-0">
          <a
            href={`/api/export/assignments/${assignment.id}/grades`}
        ...
        <div className="rounded-xl border border-outline-variant bg-surface-container-lowest p-5 shadow-2xs">
          ...
          <p className="mt-1 text-2xl font-bold text-primary">
            {stats.averageScore !== null
              ? `${stats.averageScore} / ${assignment.maxPoints}`
              : "—"}
          </p>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 rounded-2xl border border-outline-variant bg-surface-container-lowest p-6 shadow-xs space-y-3">
          ...
          <div className="pt-3 text-[11px] text-outline border-t border-outline-variant">
            ...
            {formattedPublished ? (
              <span>
                {" "}
                • {t("teacher.assignmentDetail.publishedOn", { date: formattedPublished })}
              </span>
            ) : null}
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-outline-variant bg-surface-container-lowest p-6 shadow-xs space-y-4">
          ...
          <div className="space-y-3 text-xs">
            ...
            <div>
              ...
              <span className="font-semibold text-primary">{assignment.maxPoints} pts</span>
            </div>
