...
- **Verification cadence:** do NOT run `npx tsc --noEmit`, `npm run lint`, `npm run build`, or `npm run check:i18n` after every micro-edit. Run the full verification suite (`check:i18n` + `tsc` + `lint` + `build`) only at meaningful checkpoints — at the end of each phase (R0.16, R1.x, …) or before committing a large multi-file change that crosses module boundaries. For small in-phase commits, rely on careful reading + the existing CI signal from Workers Builds. The user will explicitly request an extra check if they want one between phases.
